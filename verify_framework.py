...
def test_scenario_a():
    print("\n--- Scenario A ---")
    model = ft.build(interactive=False).data("the_pile").modelbuild("1b").type(ft.MOOE)
...
def test_scenario_b():
    print("\n--- Scenario B ---")
    ai = ft.build(interactive=False).engine("gemini").key("FAKE_KEY")
    try:
...
def test_scenario_c():
    ...
    try:
        ...
        tuner = ft.build(interactive=False).engine("hf").type("meta-llama/Meta-Llama-3-8B").data("dataset.jsonl")
